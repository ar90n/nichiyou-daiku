...
lumber_ids = st.text(min_size=1, max_size=20, alphabet='abcdefghijklmnopqrstuvwxyz0123456789_')
...
def lumber_piece(draw):
    ...
    return LumberPiece(
        id=draw(lumber_ids),
        lumber_type=draw(lumber_types),
        length=draw(lumber_lengths)
    )
...
class TestGraphProperties:
    ...
    @given(pieces=st.lists(lumber_piece(), min_size=1, max_size=10, unique_by=lambda p: p.id))
    ...
    @given(
        pieces=st.lists(lumber_piece(), min_size=2, max_size=5, unique_by=lambda p: p.id),
    ...
    @given(
        pieces=st.lists(lumber_piece(), min_size=3, max_size=10, unique_by=lambda p: p.id)
    )
    ...
    @given(
        pieces=st.lists(lumber_piece(), min_size=2, max_size=20, unique_by=lambda p: p.id)
    )
    ...
    @given(
        graph1_pieces=st.lists(lumber_piece(), min_size=1, max_size=5, unique_by=lambda p: p.id),
        graph2_pieces=st.lists(lumber_piece(), min_size=1, max_size=5, unique_by=lambda p: p.id)
    )
...
class TestGraphInvariants:
    ...
    @given(
        operations=st.lists(
            st.one_of(
                ...
                st.tuples(st.just('joint'), lumber_ids, lumber_ids, aligned_screw_joint())
            ),
            min_size=1,
            max_size=20
        )
    ...
    def test_graph_consistency_under_operations(self, operations):
        ...
        for op in operations:
            ...
            assert not any(graph.has_lumber(id) for id in set(graph._graph.nodes()) - existing_ids)
    
    @given(
        pieces=st.lists(lumber_piece(), min_size=5, max_size=10, unique_by=lambda p: p.id)
    )
...
class GraphStateMachine(RuleBasedStateMachine):
    ...
    @rule(
        src=st.sampled_from(lambda self: list(self.pieces.keys()) if self.pieces else []),
        dst=st.sampled_from(lambda self: list(self.pieces.keys()) if self.pieces else []),
    ...
    @rule(piece_id=st.sampled_from(lambda self: list(self.pieces.keys()) if self.pieces else []))
    def remove_piece(self, piece_id):
        ...
        if piece_id and piece_id in self.pieces:
            ...
            self.joints = [(s, d, j) for s, d, j in self.joints if s != piece_id and d != piece_id]
...
class TestPropertyBasedValidation:
    ...
    def test_connectivity_validation_properties(self, num_pieces, connectivity):
        ...
        from nichiyou_daiku.graph.validation import ConnectivityValidator, ValidationMode
    ...
    @given(
        pieces=st.lists(lumber_piece(), min_size=2, max_size=5, unique_by=lambda p: p.id),
        assembly_orders=st.lists(st.integers(min_value=1, max_value=10), min_size=1, max_size=10)
    )
    def test_assembly_order_validation_properties(self, pieces, assembly_orders):
        ...
        from nichiyou_daiku.graph.validation import AssemblyOrderValidator, ValidationMode
        ...
        for i, order in enumerate(assembly_orders[:len(pieces)-1]):
            ...
            if pieces[src_idx].id != pieces[dst_idx].id:
                graph.add_joint(pieces[src_idx].id, pieces[dst_idx].id, joint, assembly_order=order)
