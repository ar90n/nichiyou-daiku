            ...
            if isinstance(result, ValidationSuccess):
                ...
                for issue in result.issues:
                    ...
                    if issue.affected_pieces:
                        lines.append(f"     Affected: {', '.join(issue.affected_pieces)}")
                    if issue.suggestion:
...
class GraphValidator(ABC):
    ...
    @abstractmethod
    def validate(self, graph: WoodworkingGraph, mode: ValidationMode) -> ValidationResult:
...
class CollisionValidator(GraphValidator):
        ...
        for component in components:
            ...
            for i, piece1_id in enumerate(component):
                for piece2_id in component[i+1:]:
                    ...
                    directly_connected = (
                        graph.has_joint(piece1_id, piece2_id) or 
                        graph.has_joint(piece2_id, piece1_id)
                    )
    ...
    def _check_collision(
        ...
        if overlap_x <= self.tolerance or overlap_y <= self.tolerance or overlap_z <= self.tolerance:
            return None
...
class JointValidator(GraphValidator):
    ...
    def validate(self, graph: WoodworkingGraph, mode: ValidationMode) -> ValidationResult:
        ...
        for src_id, dst_id, edge_data in graph._graph.edges(data=True):
            ...
            face_issue = self._validate_face_compatibility(
                src_id, dst_id, joint, mode
            )
            if face_issue:
                issues.append(face_issue)
            
            # Validate edge points
            edge_issues = self._validate_edge_points(
                graph, src_id, dst_id, joint, mode
            )
    ...
    def _validate_face_compatibility(
        ...
        severity = ValidationSeverity.WARNING if mode == ValidationMode.PERMISSIVE else ValidationSeverity.ERROR
...
class StructuralValidator(GraphValidator):
    ...
    def validate(self, graph: WoodworkingGraph, mode: ValidationMode) -> ValidationResult:
        ...
        if graph.node_count() == 0:
            return ValidationSuccess(self.name, "Empty graph has no structural requirements")
