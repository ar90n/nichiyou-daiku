...
def _generate_bill_of_materials(summary: ResourceSummary) -> str:
    ...
    sorted_pieces = sorted(
        summary.pieces, 
        key=lambda p: (p.type.value, p.length)
    )
...
def _generate_shopping_list(summary: ResourceSummary) -> str:
    ...
    for piece_type in sorted(summary.pieces_by_type.keys(), key=lambda x: x.value):
        ...
        lines.append(f"- **{piece_type.value}**: {count} pieces, total {length_m:.1f} meters")
...
def _generate_purchase_recommendations(
    ...
    standard_lengths: Dict[PieceType, List[float]]
) -> str:
    ...
    for piece_type in sorted(summary.pieces_by_type.keys(), key=lambda x: x.value):
        ...
        available_lengths = standard_lengths.get(piece_type, [2440.0])  # Default fallback
        ...
        waste_percentage = (total_waste / total_purchased_length) * 100 if total_purchased_length > 0 else 0
        
        # Format available lengths
        lengths_str = ", ".join(f"{length:.0f}mm" for length in sorted(available_lengths))
...
def generate_markdown_report(
    ...
    sections.append(_generate_purchase_recommendations(resource_summary, standard_lengths))
