...
class Orientation3D(BaseModel, frozen=True):
    ...
    def orthogonalize_up(cls, up: Vector3D, info) -> Vector3D:
        ...
        return Vector3D(
            x=up_ortho_x / magnitude,
            y=up_ortho_y / magnitude,
            z=up_ortho_z / magnitude
        )
    ...
    def of(cls, *args, **kwargs) -> "Orientation3D":
        ...
        """
        if len(args) == 2 and isinstance(args[0], Vector3D) and isinstance(args[1], Vector3D):
            ...
            return cls._from_face_and_edge(args[0], args[1])
            ...
            return cls._from_euler_angles(args[0], args[1], args[2])
        elif 'euler_x' in kwargs and 'euler_y' in kwargs and 'euler_z' in kwargs:
            return cls._from_euler_angles(kwargs['euler_x'], kwargs['euler_y'], kwargs['euler_z'])
        else:
    ...
    def _from_vectors(cls, direction: Vector3D, up: Vector3D) -> "Orientation3D":
        ...
        norm_direction = Vector3D(
            x=direction.x / dir_mag,
            y=direction.y / dir_mag,
            z=direction.z / dir_mag
        )
    ...
    @classmethod
    def _from_euler_angles(cls, euler_x: float, euler_y: float, euler_z: float) -> "Orientation3D":
