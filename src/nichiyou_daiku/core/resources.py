...
class ResourceSummary(BaseModel):
    ...
    def pretty_print(self) -> str:
        ...
        for piece_type, count in sorted(self.pieces_by_type.items(), key=lambda x: x[0].value):
            total_length = self.total_length_by_type[piece_type]
            lines.append(f"  {piece_type.value}: {count} pieces (total length: {total_length}mm)")
        
        lines.extend([
            "",
            f"Total volume: {self.total_volume:,.1f} mm³"
        ])
...
def extract_resources(model: Model) -> ResourceSummary:
    ...
    for piece in model.pieces.values():
        ...
        total_length_by_type[piece.type] = total_length_by_type.get(piece.type, 0.0) + piece.length
        total_volume += volume
