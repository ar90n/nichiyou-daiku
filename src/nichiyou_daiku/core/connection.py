...
def _orientation_from_target_to_base_coords(
    ...
    if base_contact_face in ("top", "bottom") and target_contact_face in ("top", "bottom"):
        ...
        if target_face == target_edge.lhs:
            ...
            target_pair_face_in_base_coords = cross_face(reference, target_face_in_base_coords)
        else:
            # Target face is on right side - compute left side using cross product
            target_pair_face_in_base_coords = cross_face(target_face_in_base_coords, reference)
...
class Connection(BaseModel, frozen=True):
    ...
    def of(cls, base: BasePosition, target: Anchor) -> "Connection":
        ...
        target_face_in_base_coords, target_edge_in_base_coords = _orientation_from_target_to_base_coords(
